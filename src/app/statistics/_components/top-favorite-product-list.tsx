...
import { useTopFavoriteProductListQuery } from '@/queries';
import { TopFavoriteProductResType } from '@/types';
import { formatMoney, renderImageUrl } from '@/utils';
import { Heart } from 'lucide-react';
import Image from 'next/image';
import { useInView } from 'react-intersection-observer';

export default function TopFavoriteProductList() {
  const { ref, inView } = useInView({
    triggerOnce: true,
    rootMargin: '100px'
  });
  const { data, isLoading } = useTopFavoriteProductListQuery({
    enabled: inView
  });
  const products: TopFavoriteProductResType[] = data?.data?.content || [];
              ...
              >
                ...
                <div className='min-w-0 flex-1'>
                  ...
                  <div className='flex items-center gap-x-2'>
                    ...
                    {product.totalFavorites} yêu thích
                  </div>
                </div>
              </div>
            );
