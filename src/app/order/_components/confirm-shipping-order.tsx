...
import { CMD_START_SHIPPING } from '@/constants';
...
export default function ConfirmShippingOrder({ orderId }: { orderId: string }) {
  ...
  const handleConfirmShippingOrder = async () => {
    await updateOrderStatusMutation.mutateAsync(
      {
        cmd: CMD_START_SHIPPING,
        id: orderId
      },
      {
        onSuccess: (res) => {
          if (res.result) {
            notify.success('Cập nhật trạng thái đang giao hàng thành công');
  ...
  return (
    <AlertDialog>
      <AlertDialogTrigger asChild>
        <span>
          <Button variant={'primary'}>Đang giao hàng</Button>
        </span>
      </AlertDialogTrigger>
      <AlertDialogContent className='data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-0! data-[state=closed]:slide-out-to-top-0! data-[state=open]:slide-in-from-left-0! data-[state=open]:slide-in-from-top-0! top-[30%] p-4'>
        <AlertDialogHeader>
          <AlertDialogTitle className='flex items-center gap-2 text-sm font-normal'>
            ...
            Bạn có chắc chắn đơn hàng này đang được giao không?
          </AlertDialogTitle>
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction
            ...
            asChild
          >
            <Button variant={'primary'} onClick={handleConfirmShippingOrder}>
